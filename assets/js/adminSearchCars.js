...
const searchCars = (queryParams = '') => {
  ...
  const { token } = getUser;
  const endpoint = `https://mthamayor-auto-mart.herokuapp.com/api/v1/car?${queryParams}`;
  const fetchRequest = {
    method: 'GET',
    headers: {
...
if (getUser.is_admin !== true) {
  Populator.showStickyNotification(
    'error',
    'You do not have permission to see this page. redirecting to dashboard in 3 seconds',
  );
  Helpers.delayedRoute(3000, 'dashboard.html');
  throw new Error('403');
}
...
searchForm.addEventListener('submit', (event) => {
  ...
  if (String(vehicleStatus).length > 0) {
    queryParam.status = String(vehicleStatus).toLowerCase();
  }
